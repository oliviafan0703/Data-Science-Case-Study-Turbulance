...
St_summary <- tibble(min=min(data_train$St), Q1=quantile(data_train$St, 0.25), median=median(data_train$St),mean=mean(data_train$St),Q3=quantile(data_train$St, 0.75),max=max(data_train$St))
St_summary%>%kable(digits=3,caption="St summary statistics")
Re_summary <- tibble(min=min(data_train$Re), Q1=quantile(data_train$Re, 0.25), median=median(data_train$Re),mean=mean(data_train$Re),Q3=quantile(data_train$Re, 0.75),max=max(data_train$Re))
Re_summary%>%kable(digits=3,caption="Re summary statistics")
Fr_summary <- tibble(min=min(data_train$Fr), Q1=quantile(data_train$Fr, 0.25), median=median(data_train$Fr),mean=mean(data_train$Fr),Q3=quantile(data_train$Fr, 0.75),max=max(data_train$Fr))
Fr_summary%>%kable(digits=3,caption="Fr summary statistics")
```
...
```{r, echo=F, fig.width=6, fig.height=4, warning=F} 
library(gridExtra)
