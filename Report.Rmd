...
predict(ridge.test2, type = 'coefficients', s = best_lambda)
```
...
final_model_M4 <- lm(C_moment_4 ~ Re_category+Fr_category+Fr_category*Re_category+St*Re_category, data = data_train)
summ(final_model_M1)
summ(final_model_M2)
summ(final_model_M3)
summ(final_model_M4)
...
data_predicted_output <- data_test_transformed%>%select(St, Re, Fr, Predicted_R_M1, Predicted_R_M2, Predicted_R_M3, Predicted_R_M4)
...
Since our response variables, where highly linearly correlated, we came to the conclusion that the model which best fit the 

```{r}
lm_test <- lm(C_moment_2 ~ St + Re_category + Fr_transformed, data = data_train)
summary(lm_test)
```
